        ...
        while n_steps < n_rollout_steps:
            ...
            for i, info in enumerate(infos):
                if (info.get('episode') is not None):
                    if (info.get('episode')['l']==2000): # should fix to env max lenght
                        dones[i] = False
        ...
        while self.num_timesteps < total_timesteps:
            ...
            if log_interval is not None and iteration % log_interval == 0:
                ...
                if len(self.specific_reward_info_buffer) > 0 and len(self.specific_reward_info_buffer[0]) > 0:
                    ...
                    logger.record('rollout/mimic_base_pose_reward', safe_mean([specific_reward_info['mimic_base_pose_reward'] for specific_reward_info in self.specific_reward_info_buffer]))
