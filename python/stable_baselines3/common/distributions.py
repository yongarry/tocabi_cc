...
class DiagGaussianDistribution(Distribution):
    ...
    def proba_distribution_net(self, latent_dim: int, log_std_init: float = 0.0) -> Tuple[nn.Module, nn.Parameter]:
        ...
        log_std = nn.Parameter(th.log(th.ones(self.action_dim) * 10.0), requires_grad=False)
        # noise = th.tensor([0.02, # phase
        #                      0.01, 0.01, 0.01, 0.08, 0.08, 0.08, # com (position + orientation)
        #                      0.01, 0.01, 0.01, 0.08, 0.08, 0.08]) # support foot (orientation)
        # log_std = nn.Parameter(th.log(th.ones(self.action_dim) * noise), requires_grad=False)
